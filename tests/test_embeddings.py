...
from image_latent_transformer.embeddings import (
    unpack_bits,
    PatchedBitEmbeddings,
    patch_embedding_layer,
    join_embedding_layer
)
...
class TestEmbeddings:
    ...
    def test_embedding_setup(self, model):
        ...
        patch_embedding_layer(model)
        patched_embeddings = model.get_input_embeddings()
        assert isinstance(patched_embeddings, PatchedBitEmbeddings)
        assert isinstance(patched_embeddings.embedding, nn.Embedding)
        assert patched_embeddings.embedding.num_embeddings == 256
    ...
    def test_parameter_count_preservation(self, model):
        ...
        embedding_dim = patched_embeddings.embedding.embedding_dim
    ...
    def test_patched_bit_embeddings_properties(self, model):
        ...
        assert hasattr(patched_embeddings, 'embedding')
        assert hasattr(patched_embeddings, 'bit_proj')
        
        weight = patched_embeddings.weight
        assert weight.shape == (256, patched_embeddings.embedding.embedding_dim)

    def test_bit_projection_initialization(self, model):
        patch_embedding_layer(model)
        patched_embeddings = model.get_input_embeddings()
        
        assert torch.allclose(patched_embeddings.bit_proj.weight, torch.zeros_like(patched_embeddings.bit_proj.weight))
    ...
    def test_weight_parameter_properties(self, model):
        patch_embedding_layer(model)
        patched_embeddings = model.get_input_embeddings()
        
        # Test that weight is a proper parameter
        assert isinstance(patched_embeddings.weight, nn.Parameter)
        assert patched_embeddings.weight.requires_grad
        assert patched_embeddings.weight.shape == (256, patched_embeddings.embedding.embedding_dim)
